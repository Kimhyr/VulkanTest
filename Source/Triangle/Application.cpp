...
Void Triangle::Application::initWindow() {
    vkfw::init();
    vkfw::setErrorCallback((GLFWerrorfun) Triangle::Application::error_window_callback);
    vkfw::WindowHints winHints; // TODO Redo this.
    winHints.resizable = vkfw::eTrue;
    winHints.visible = vkfw::eFalse;
    winHints.maximized = vkfw::eTrue;
...
Void Triangle::Application::initVulkan() {
...
#ifdef TRIANGLE_APPLICATION_VALIDATION_LAYERS_ENABLED
Void Triangle::Application::initDebugMessenger() {
...
#ifdef TRIANGLE_APPLICATION_VALIDATION_LAYERS_ENABLED
Bool Triangle::Application::checkValidationLayerSupport() {
    UInt32 layerCount = 0;
    ...
    for (
            UInt32 i = 0; i < Triangle::Application::ValidationLayerCount; ++i
            ) {
        for (
                UInt32 j = 0; j < layerCount; ++j
                ) {
...
std::vector<const Char8 *> Triangle::Application::getRequiredExtensions() {
    UInt32 gextCount = 0;
    const Char8 **gexts;
